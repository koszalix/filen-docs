...
The metadata of files (including name, MIME type, size, timestamp and encryption key) and directories (its name) is always client-side encrypted using the master key, as described in the [Encryption guide](./encryption#metadata-encryption). 

#### File chunks

The binary datas of files is saved in chunks of 1MiB. The chunk's data is encrypted as described in the [Encryption guide](./encryption#data-encryption). A randomly generated encryption key is used for each file's data. 
...
To decrypt the chunk data, use [data decryption](./encryption#data-decryption). The encryption key used here is from the file metadata's encrypted `metadata`.
...
To encrypt a chunk, use [data encryption](./encryption#data-encryption) with the encryption key generated above.
